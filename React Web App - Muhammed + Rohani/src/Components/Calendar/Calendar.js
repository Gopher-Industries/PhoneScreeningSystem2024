...
import "./Calendar.css"; // Import the CSS that styles both sidebars
...
class CalendarWithSidebar extends Component {
  ...
  componentDidMount() {
    // Add the Material Icons link dynamically
    ...
    if (start && end && title) {
      const updatedEvents = events.map((event) =>
        event.id === clickedEvent.id
          ? { ...event, title, desc, start: start.toDate(), end: end.toDate(), color }
          : event
      );
      ...
      <div className="notes-section">
        ...
        {filteredEvents.map((event, index) => (
          <div key={index} className="event-note">
            <p>
              {event.title} : {moment(event.start).format("h:mm a")} -{" "}
  ...
  render() {
    return (
      <LocalizationProvider dateAdapter={AdapterMoment}>
        <div className="main-container">
          ...
          <aside id="sidebar">
            ...
            <div className="logo-container">
              <a href='/'><img src={logo} alt="Logo" className="logo-img2" /></a>
            </div>
        ...
        <Dialog
          open={this.state.openSlot || this.state.openEvent}
          onClose={this.handleClose}
        >
          <h2>
            {this.state.openSlot ? "Book an appointment" : "View/Edit Appointment"}{" "}
            on {moment(this.state.start).format("MMMM Do YYYY")}
          </h2>
          ...
          <div className="dialog-buttons">
            ...
            {this.state.openEvent && <Button onClick={this.deleteEvent} color="secondary">Delete</Button>}
            <Button
              onClick={this.state.openSlot ? this.setNewAppointment : this.updateEvent}
              color="primary"
            >
