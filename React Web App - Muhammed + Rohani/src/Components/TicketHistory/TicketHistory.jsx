...
const TicketHistory = () => {
  ...
  useEffect(() => {
    ...
    // Calculate status counts based on the table data
    calculateStatus();
  }, []);
  ...
  const calculateStatus = () => {
    ...
    // Example to scan the table data
    const rows = document.querySelectorAll('#example tbody tr');
    rows.forEach((row) => {
      const status = row.cells[5].textContent.trim(); // Get the status from the 6th cell (index 5)
        ...
        <aside id="sidebar" className="ticket-history-page">
          ...
          <ul className="sidebar-list">
            <li className="sidebar-list-item">
              <span className="material-icons-outlined">support_agent</span> View The Organisations Assigned Tickets
            </li>
          </ul>
        </aside>
